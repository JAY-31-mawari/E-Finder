...
var charging_login = new mongoose.Schema({
    email:String,
    password:String
})

var log = new mongoose.model('log' , charging_login);
...
app.post('/login', async (req, res) => {
    try {
        ...
        if (username.password === password) {
            ...
            res.sendFile(path.join(__dirname + '/views/Error_home.html'))
        }
    }
    catch {
...
app.post('/subscription1', (req, res) => {
    ...
    data.save().then(() => {
        res.sendFile(path.join(__dirname + '/views/sub_message.html'));
    }).catch(() => {
        res.send("Error ! Can't connect to the database");
    })
})
